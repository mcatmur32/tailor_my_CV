...
class Database:
        ...
        if cur.fetchone()["n"] == 0:
            ...
            samples = [
                ("SpaceTech Ltd", "RF Graduate Engineer", today, "Submitted",
                 os.path.join(os.getcwd(), "SpaceTech_RF_CoverLetter.pdf"), os.path.join(os.getcwd(), "SpaceTech_RF_CoverLetter.pdf"), os.path.join(os.getcwd(), "SpaceTech_RF_CoverLetter.pdf")),
                ("PhotonWorks", "Applied Physicist (DFT)", today, "Draft",
                 os.path.join(os.getcwd(), "PhotonWorks_DFT_Notes.pdf"), os.path.join(os.getcwd(), "SpaceTech_RF_CoverLetter.pdf"), os.path.join(os.getcwd(), "SpaceTech_RF_CoverLetter.pdf")),
                ("Aerosys", "Seeker Algorithms Intern", today, "Interview",
                 os.path.join(os.getcwd(), "Aerosys_Interview_Prep.pdf"), os.path.join(os.getcwd(), "SpaceTech_RF_CoverLetter.pdf"), os.path.join(os.getcwd(), "SpaceTech_RF_CoverLetter.pdf")),
            ]
            self.conn.executemany("""
                INSERT INTO applications (company, role, deadline, status, JD_file_path, CV_file_path, CL_file_path)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, samples)
            self.conn.commit()
    ...
    def fetch_all(self):
        cur = self.conn.execute("""
            SELECT id, company, role, deadline, status, JD_file_path, CV_file_path, CL_file_path
            FROM applications
            ORDER BY id DESC
        """)
        return cur.fetchall()
    ...
    def add_JD_file_path(self, app_id: int, JD_file_path: str):
        self.conn.execute("UPDATE applications SET JD_file_path=? WHERE id=?", (JD_file_path, app_id))
        self.conn.commit()
    ...
    def get_JD_file_path(self, app_id: int):
        cur = self.conn.execute("SELECT JD_file_path FROM applications WHERE id=?", (app_id,))
        row = cur.fetchone()
        return None if row is None else row["JD_file_path"]
